"""
Dependency Agent — scans dependency files against OSV index.

Copied from api/adk_agents/dependency_agent.py for standalone CLI usage.
Resolves data paths relative to project root. Loads osv_index.pkl lazily
ONLY when dependency files exist.
"""
...
# Resolve data paths relative to project root (PBL-2/)
_PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
OSV_DIR = os.path.join(_PROJECT_ROOT, "data", "osv")
INDEX_PATH = os.path.join(_PROJECT_ROOT, "data", "osv_index.pkl")

_index_cache: dict[str, dict[str, list[dict[str, Any]]]] | None = None


# -------------------------
# OSV INDEX HANDLING (pickle-only in CLI)
# -------------------------
...
def _build_and_save_index():
    ...
    # CLI scanner is expected to use the precomputed pickle shipped
    # with the repository. We keep this helper for completeness
    # but in CLI mode we do not regenerate the index from raw JSON.
    return index
...
def _load_index():
    ...
    if os.path.exists(INDEX_PATH):
        try:
            ...
            # If the pickle cannot be read, fail closed with an empty
            # index instead of trying to rebuild from JSON.
            _index_cache = {"PyPI": {}, "npm": {}, "RubyGems": {}}
            return _index_cache

    # No pickle present — return empty index to keep CLI lightweight.
    _index_cache = {"PyPI": {}, "npm": {}, "RubyGems": {}}
    return _index_cache
...
def scan_dependencies(repo_path: str):
    # Lazy load: only load the index if dep files exist
